...
float getShadow(vec3 P, vec3 N){
    ...
    for(int i=0;i<S;i++){
        ...
        sh += smoothstep(recDepth - uBiasBase - 0.002, recDepth - uBiasBase + 0.002, d);
    }
    return sh/float(S);
}
...
void main(){
    ...
    vec3 baseColor = albedoTex.rgb * 1.2;
    ...
    vec3 P = (invV * vec4(fragPosV, 1.0)).xyz;
